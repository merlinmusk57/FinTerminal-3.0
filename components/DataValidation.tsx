...
const ValidationCell: React.FC<ValidationCellProps> = ({ metric, bank, selectedPeriod, selectedSegment, selectedCurrency, fxRate, dataPoints }) => {
  ...
  // Ref to track latest comment text for saving on unmount/click-outside
  const commentTextRef = useRef('');

  // Use Logical ID for Validation Status
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (isCommentOpen && commentRef.current && !commentRef.current.contains(event.target as Node)) {
        // Save comment before closing if clicked outside
        saveComment();
      }
    }
  ...
  }, [isCommentOpen, data, savedComment]); // Dependencies needed to ensure saveComment has context
  ...
  if (!data) return <td className="p-3 border-l border-gray-800 bg-slate-950/50"></td>;


  // --- Consistency Logic with Comparison Tab ---
  // Check if value is effectively zero for breakdown metrics (Legacy N/A check)
  ...
  // If explicitly N/A (via validation) or strictly 0 in breakdown/ratio (legacy N/A)
  ...
  const handleStartEdit = () => {
      if (currentStatus.isValidated) return; // Locked
      // For editing, we populate with the DISPLAY value (so user edits what they see)
      setLocalValue(showNA ? '0' : displayVal.toFixed(2)); // Use fixed for editing precision
      setIsEditing(true);
  };
  ...
  const handleCommitEdit = () => {
      ...
      // Convert back to Base Currency for storage if needed
      let storageVal = numVal;
      if (unit !== '%') {
          if (data.currency === 'HKD' && targetUSD) {
             // User typed USD, convert back to HKD
             storageVal = numVal * fxRate;
          } else if (data.currency === 'USD' && !targetUSD) {
             // User typed HKD, convert back to USD
             storageVal = numVal / fxRate;
          }
      }
  ...
  // Toggle Flag (Red/Bold) - Was previously Toggle NA
  ...
  const onCitationClick = (e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
      // Use specific data point ID for the citation filter in audit trail
  ...
  return (
    <td className={`p-3 border-l border-gray-800 transition-colors relative group min-w-[140px] ${isLocked ? 'bg-green-900/10' : (isModified ? 'bg-amber-900/10' : 'hover:bg-gray-800/30')}`}>
      <div className="flex items-center justify-between gap-2 h-6">
        
        {/* Left Action: Cross (Exclude) Toggle & Comment */}
        <div className="flex items-center gap-1">
             <div className="w-5 flex items-center justify-center">
                {!isLocked && (
                    <button 
                        ...
                        type="button"
                        className={`p-1 rounded hover:bg-red-900/30 transition-colors opacity-0 group-hover:opacity-100 ${isFlagged ? 'text-red-500 bg-red-900/10 opacity-100' : 'text-gray-600 hover:text-red-500'}`}
                        title={isFlagged ? "Include Data" : "Exclude Data"}
                    >
                        {/* CHANGED: Flag icon to X icon */}
                        <X size={12} strokeWidth={3} className={isFlagged ? "text-red-500" : ""} />
                    </button>
                )}
             </div>
             ...
             <div className="relative" ref={commentRef}>
                <button 
                    ...
                    type="button"
                    className={`p-1 rounded hover:bg-blue-900/30 transition-colors ${hasComment ? 'text-blue-400 opacity-100' : 'text-gray-600 opacity-0 group-hover:opacity-100 hover:text-blue-400'}`}
                    title="Add/Edit Comment"
                >
                    <MessageSquare size={12} fill={hasComment ? "currentColor" : "none"} />
                </button>
                {isCommentOpen && (
                    <div className="absolute top-full left-0 z-50 bg-gray-900 border border-gray-700 p-2 rounded shadow-xl w-64 mt-2">
                        <textarea 
        ...
        <div className="flex-1 text-right flex items-center justify-end gap-1 relative">
            ...
            {/* Citation Icon (Appears on Hover) */}
            <button 
                onClick={onCitationClick}
                className="opacity-0 group-hover:opacity-100 text-gray-600 hover:text-amber-500 transition-all transform hover:scale-110 mr-1"
            ...
            {isEditing ? (
                <input 
                    ...
                    className="w-full bg-slate-900 border border-amber-500 text-right text-white text-sm px-1 py-0.5 rounded outline-none font-mono shadow-inner"
                />
            ) : (
                <div 
                    onClick={handleStartEdit}
                    className={`text-sm font-mono flex justify-end items-center gap-1 ${!isLocked ? 'cursor-text hover:text-white' : 'cursor-default'} ${showNA ? 'text-gray-500' : (isLocked ? 'text-green-400 font-bold' : (isFlagged ? 'text-red-500 font-extrabold' : (isModified ? 'text-amber-400' : 'text-gray-300')))}`}
                >
            ...
            {/* Dropdown Trigger for N/A */}
            {!isLocked && !isEditing && (
                <div className="relative" ref={dropdownRef}>
                    <button 
                        onClick={(e) => { e.stopPropagation(); setIsDropdownOpen(!isDropdownOpen); }}
                        className={`p-0.5 ml-1 rounded hover:bg-gray-700 text-gray-500 hover:text-white transition-colors ${isDropdownOpen ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                    >
                        <ChevronDown size={10} />
                    </button>
                    {isDropdownOpen && (
                        <div className="absolute top-full right-0 mt-1 w-28 bg-gray-900 border border-gray-700 rounded shadow-xl z-50 overflow-hidden">
                             <button 
                                onClick={onSetNA}
        ...
        {/* Right Action: Validate / Lock */}
        <div className="w-5 flex items-center justify-center">
            {isLocked ? (
                // LOCKED STATE: Show Lock (Click to Unlock)
                <button 
                    onClick={onToggleValidation}
                    type="button"
                    className="p-1 rounded bg-green-900/20 text-green-500 hover:bg-green-900/40 transition-colors"
                    title="Unlock Value"
                >
                    <Lock size={12} />
                </button>
            ) : (
                // UNLOCKED STATE: Show Tick (Validate)
                 <button 
                    onClick={onToggleValidation}
                    type="button"
                    className="p-1 rounded hover:bg-green-900/30 text-gray-600 hover:text-green-500 transition-colors opacity-0 group-hover:opacity-100"
                    title="Validate & Lock"
                >
                    <Check size={12} />
                </button>
...
export const DataValidation: React.FC = () => {
  ...
  const handleExport = () => {
     // Simple HTML Table export logic matching Comparison Tab
  ...
  const Row = ({ label, isChild = false, hasChildren = false, rowKey = '', isTotal = false }: any) => (
    <tr className={`border-b border-gray-800 last:border-0 hover:bg-gray-800/20 transition-colors ${isTotal ? 'bg-gray-900/60' : ''}`}>
      <td className={`p-3 text-sm text-gray-300 border-r border-gray-800 sticky left-0 z-20 ${isTotal ? 'bg-slate-900' : 'bg-slate-950'}`}>
         <div className={`flex items-center w-full ${isChild ? 'pl-12' : 'pl-2'}`}>
            ...
            <span className={`${isTotal ? 'text-amber-500 font-bold' : 'text-gray-300'} ${hasChildren ? 'font-bold' : ''}`}>{label}</span>
         </div>
      </td>
  ...
  const SectionHeader = ({ title }: { title: string }) => (
    <tr className="bg-gray-900 border-b border-gray-800 sticky z-10">
      <td className="p-2 pl-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest border-l-4 border-blue-600 sticky left-0 bg-gray-900 z-20">{title}</td>
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950">
        {/* Header / Toolbar (Matched with Comparison Tab) */}
        <div className="p-4 border-b border-gray-800 bg-slate-900 flex flex-wrap items-center gap-4 z-50 relative shadow-md">
             <div className="flex items-center gap-2 border-r border-gray-700 pr-4">
                 <div className="flex flex-col">
                    <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">Data Validation</h2>
             ...
             <div className="flex items-center gap-2">
                ...
                <select value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)} className="bg-gray-800 border border-gray-700 text-white text-xs rounded px-2 py-1 outline-none focus:border-amber-500">
             ...
             <div className="flex items-center gap-2">
                ...
                <select value={selectedCurrency} onChange={(e) => setSelectedCurrency(e.target.value)} className="bg-gray-800 border border-gray-700 text-white text-xs rounded px-2 py-1 outline-none focus:border-amber-500">
            ...
            <div className="relative" ref={bankDropdownRef}>
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setIsBankDropdownOpen(!isBankDropdownOpen)} className="flex items-center gap-2 bg-gray-800 border border-gray-700 text-white text-xs rounded px-3 py-1 hover:border-amber-500 transition-colors">
                ...
                {isBankDropdownOpen && (
                    <div className="absolute top-full left-0 mt-1 w-64 bg-gray-900 border border-gray-700 rounded shadow-xl z-50 p-1">
                        {Object.values(BankName).map(bank => (
                            <div key={bank} onClick={() => toggleBank(bank)} className="flex items-center gap-2 p-2 hover:bg-gray-800 cursor-pointer rounded text-xs text-gray-300">
            ...
            <div className="flex items-center gap-2 pl-4 border-l border-gray-700">
                ...
                <select value={selectedSegment} onChange={(e) => setSelectedSegment(e.target.value as StandardizedSegment)} className="bg-gray-800 border border-gray-700 text-white text-xs rounded px-2 py-1 outline-none focus:border-amber-500 w-48">
             ...
             <div className="flex gap-4 text-[10px] font-mono text-gray-500 bg-gray-900 px-3 py-1 rounded border border-gray-800">
