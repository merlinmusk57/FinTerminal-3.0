...
const CitationCell: React.FC<CitationCellProps> = ({ metric, bank, selectedPeriod, selectedSegment, selectedCurrency, fxRate, dataPoints }) => {
  ...
  if (!data) return <td className="p-3 border-l border-gray-200 dark:border-gray-800 text-right">-</td>;
  ...
  if (isFlagged) {
      return <td className="p-3 text-right font-mono border-l border-gray-200 dark:border-gray-800 relative group min-w-[140px] text-gray-400 dark:text-gray-700">-</td>;
  }
  ...
  if (!isVisible && !isNA) {
      return (
          <td className="p-3 text-right font-mono border-l border-gray-200 dark:border-gray-800 relative group min-w-[140px]">
              <span className="text-gray-400 dark:text-gray-600 text-xs italic">Pending</span>
          </td>
      );
  }
  ...
  const formatValue = (v: number) => {
    if (showNA) return <span className="text-gray-400 dark:text-gray-600 font-normal">N/A</span>;
      ...
      <div className="flex items-center justify-end gap-1">
        <div className="relative mr-auto" ref={commentRef}>
             <button 
                ...
                className={`transition-colors ${hasComment ? 'text-blue-500 dark:text-blue-400 opacity-100' : 'text-gray-400 dark:text-gray-600 opacity-0 group-hover:opacity-100 hover:text-blue-500'}`}
                title="View/Add Comment"
             >
                 <MessageSquare size={12} fill={hasComment ? "currentColor" : "none"} />
             </button>
             {isCommentOpen && (
                <div className="absolute top-full left-0 z-50 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 p-2 rounded shadow-xl w-64 mt-1 text-left">
                    <textarea 
        ...
        <span className={`
            ${currentValue < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-gray-200'} 
            ${isValidated ? 'font-bold' : ''} 
            ${isFlagged ? 'text-red-600 dark:text-red-500 font-extrabold' : ''}
            ${!isFlagged && isOverride && !isValidated ? 'text-amber-600 dark:text-amber-400' : ''}
        `}>
            {formatValue(val)}
        </span>
      </div>
...
export const Comparison: React.FC = () => {
  ...
  const ComparisonRow = ({ label, isChild = false, hasChildren = false, rowKey = '', isTotal = false }: any) => (
    <tr className={`hover:bg-gray-100 dark:hover:bg-gray-800/30 border-b border-gray-200 dark:border-gray-800 last:border-0 transition-colors ${isTotal ? 'bg-gray-50 dark:bg-gray-900/60 font-bold' : ''}`}>
      <td className={`p-3 text-sm text-gray-700 dark:text-gray-300 border-r border-gray-200 dark:border-gray-800 sticky left-0 z-20 ${isTotal ? 'bg-white dark:bg-slate-900' : 'bg-white dark:bg-slate-950'}`}>
         <div className={`flex items-center w-full ${isChild ? 'pl-12' : 'pl-2'}`}>
            ...
            <span className={`${isTotal ? 'text-amber-600 dark:text-amber-500' : 'text-gray-700 dark:text-gray-300'} ${hasChildren ? 'font-bold' : ''}`}>{label}</span>
         </div>
      </td>
  ...
  const SectionHeader = ({ title }: { title: string }) => (
    <tr className="bg-gray-100 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky z-10">
      <td className="p-2 pl-4 text-[10px] font-bold text-gray-600 dark:text-gray-500 uppercase tracking-widest border-l-4 border-amber-600 sticky left-0 bg-gray-100 dark:bg-gray-900 z-20">{title}</td>
    ...
    <div className="h-full flex flex-col">
      <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-900 flex flex-wrap items-center gap-4 z-50 relative shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <select value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)} className="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded px-2 py-1 outline-none focus:border-amber-500">
            {periods.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500 uppercase font-mono">Currency:</span>
          <select value={selectedCurrency} onChange={(e) => setSelectedCurrency(e.target.value)} className="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded px-2 py-1 outline-none focus:border-amber-500">
        ...
        <div className="relative">
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => setIsBankDropdownOpen(!isBankDropdownOpen)} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded px-3 py-1 hover:border-amber-500 transition-colors">
            ...
            {isBankDropdownOpen && (
                <div className="absolute top-full left-0 mt-1 w-64 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded shadow-xl z-50 p-1">
                    {Object.values(BankName).map(bank => (
                        <div key={bank} onClick={() => toggleBank(bank)} className="flex items-center gap-2 p-2 hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer rounded text-sm text-gray-700 dark:text-gray-300">
        ...
        <div className="flex items-center gap-2 pl-4 border-l border-gray-300 dark:border-gray-700">
          ...
          <select value={selectedSegment} onChange={(e) => setSelectedSegment(e.target.value as StandardizedSegment)} className="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded px-2 py-1 outline-none focus:border-amber-500 w-48">
      ...
      <div className="flex-1 overflow-auto bg-white dark:bg-slate-950 relative transition-colors duration-300">
        ...
        <div className="p-2 border-t border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-gray-900/50 flex flex-col gap-1 text-[10px] text-gray-500 font-mono z-30">
